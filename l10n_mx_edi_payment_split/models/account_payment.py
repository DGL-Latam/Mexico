...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _getMoves(self):
        payment_move_ids = self.env['account.move'].browse(self._context.get('active_ids', []))

    @api.onchange('csv_file')
    def l10n_mx_edi_import_invoice_info(self):
        """csv file must be ---> move_number,amount,rate"""
        self.ensure_one()
        ...
        try:
            ...
            raise UserError(_("Please use a file with CSV format."))
        register_ids = self.payment_move_ids
        for row in csv_iterator:
            invoice = self.invoice_ids.search([('name', '=', row[0]), ('state', '=', 'open')])
            try:
                col_num = len(row)
            ...
            if not invoice or amount <= 0:
                continue
            register_ids |= self.payment_move_ids.create({
                'move_id': invoice.id,
        ...
        if not register_ids:
            raise UserError(_("No invoice info to import. Please make sure you're using a file with CSV format "
                              "and comma (,) separator."))
        self.update({'payment_move_ids': [(6, 0, register_ids.ids)]})
        self.invoice_ids = register_ids.mapped('move_id')
        self._onchange_payment_invoice()
        # update context
        if register_ids:
            ctx = self._context.copy()
            ctx.update({'active_ids': register_ids.mapped('move_id.id'), 'active_id': register_ids[0].id})
            self = self.with_context(ctx)

    @api.model
    def default_get(self, values):
        res = super(AccountPaymentRegister, self).default_get(values)
        payment_currency_id = self.env['res.currency'].browse(res.get('currency_id'))

        invoice_ids = self.env['account.move'].browse(self._context.get('active_ids')).sorted('due_date')
        amount_dict = dict(
    ...
    def _onchange_amount(self):
        ...
        if self.amount != self.dummy_amount:
            self._onchange_payment_invoice()
        return super(AccountPaymentRegister, self)._onchange_amount()

    @api.onchange('payment_move_ids', 'custom_rate')
    def _onchange_payment_invoice(self):
        if self.amount == self.dummy_amount:
            cumulative_amount = sum(
                self.payment_move_ids.mapped('payment_currency_amount'))
        ...
        for line in self.payment_move_ids.sorted('date_due'):
    ...
    def _onchange_currency_at_payment(self):
        ...
        for line in self.payment_move_ids:
            ...
            if self.currency_id == line.move_id.currency_id:
                line.rate = 1.00
                continue
            rate = self.custom_rate
            if not self.custom_rate:
                rate = self.currency_id._convert(
                    1, line.move_id.currency_id, line.move_id.company_id,
                    self.payment_date, round=False)
                same_currency = line.move_id.currency_id == company_currency
    ...
    def create_payments(self):
        ...
        for payment_vals in self.get_payments_vals():
            ...
            for line in self.payment_invoice_ids:
                ...
                if line.move_id.id not in payment_vals['invoice_ids'][0][2]:
                    continue

                inv_list[line.move_id.id] = {
                    'currency_id': line.move_id.currency_id,
                    'type': line.move_id.type,
                    ...
                    'rate': line.rate,
                    'residual': line.move_id.residual,
                }
