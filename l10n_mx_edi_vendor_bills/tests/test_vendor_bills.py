...
class MxEdiVendorBills(TransactionCase):
    ...
    def test_008_ecc12(self):
        """Create a vendor bill with ECC12 complement and check its values"""
        self.env['ir.config_parameter'].create({
            'key': 'l10n_mx_product_ecc12',
            'value': self.product.id,
        })
        # create invoice
        xml_str = misc.file_open(os.path.join(
            'l10n_mx_edi_vendor_bills', 'tests', 'ecc12.xml')).read()
        self.attach_wizard_obj.create_partner(base64.b64encode(xml_str.encode('UTF-8')), self.key)
        res = self.attach_wizard_obj.check_xml({self.key: base64.b64encode(xml_str.encode('UTF-8'))})
        invoices = res.get('invoices', {})
        inv_id = invoices.get(self.key, {}).get('invoice_id', False)
        ...
        # check values
        inv = self.invoice_obj.browse(inv_id)
        cfdi = fromstring(xml_str.encode())
        xml_amount = float(cfdi.get('Total', 0.0))
        complement_amount = float(self.attach_wizard_obj._get_fuel_complement(cfdi).get('Total', 0.0))
        self.assertEqual(int(inv.amount_total), int(round(xml_amount + complement_amount, 2)), "Error: Total amount")
