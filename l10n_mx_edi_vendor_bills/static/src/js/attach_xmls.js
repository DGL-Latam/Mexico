...
var attachXmlsWizard = FieldChar.extend({
    ...
    sendFileToServer: function(files){
        ...
        ctx.partner_bank_id = options.partner_bank_id;
    ...
    wrongMsgXml: function(file, able_buttons){
        ...
        $.each(file, function(ikey, val){
            ...
            if(ikey === 'supplier'){
                ...
                errors += _t('<div><span level="1">An invoice was found with similar information to the XML attached, but the XML Folio</span> does not match with <span level="1">Supplier Invoice Number</span>: ') +
