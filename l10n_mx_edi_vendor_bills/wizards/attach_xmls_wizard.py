...
class AttachXmlsWizard(models.TransientModel):
    ...
    partner_bank_id = fields.Many2one(
        "res.partner.bank",
        help="Bank account setted here will be selected on the invoice",
    )
    ...
    def get_impuestos(self, xml):
        ecc12 = self._context.get('force_taxes_complement')
        if not hasattr(xml, 'Impuestos') and not ecc12:
            return {}
        taxes = []
        taxes_list = {'wrong_taxes': [], 'taxes_ids': {}, 'withno_account': []}
        if ecc12:
            concepts = enumerate(xml.Conceptos.ConceptoEstadoDeCuentaCombustible)
        else:
            concepts = enumerate(xml.Conceptos.Concepto)
        for index, rec in concepts:
            if not hasattr(rec, 'Impuestos') and not hasattr(rec, 'Traslados'):
                continue
            taxes_list['taxes_ids'][index] = []
            taxes_xml = rec.Impuestos if hasattr(rec, 'Impuestos') else rec
            ...
            for tax in taxes:
                ...
                if (not tax_group_id or not tax_get) and tax.get('tax', False) not in taxes_to_omit:
                    ...
                    continue
                if not tax_get.account_id.id and tax.get('tax', '') not in taxes_to_omit:
        ...
        if hasattr(local_taxes, 'RetencionesLocales'):
            for local_ret in local_taxes.RetencionesLocales:
                ...
                if not tax and name not in taxes_to_omit:
                    taxes_list['wrong_taxes'].append(name)
                    continue
                if tax and not tax.account_id and name not in taxes_to_omit:
    ...
    def validate_documents(self, key, xml, account_id):
        ...
        use_default_partner = self.env['ir.config_parameter'].sudo().get_param(
            'force_partner_wizard_vendor_bills', '')
        if use_default_partner and self._context.get('partner_id'):
            ...
            exist_supplier = (partner_obj.search(
                ['&', ('vat', '=', xml_vat_emitter), '|',
                 ...
                 '|', ('company_id', '=', False),
                 ('company_id', '=', self.env.user.company_id.id)], limit=1) or
                partner_obj.browse(self._context.get('partner_id')))
    ...
    def create_invoice(
        ...
        bank_account = self._context.get('partner_bank_id', False)
        ...
        invoice_line_ids = self._get_concepts(xml, journal, supplier, account_id, taxes)
        ...
        if bank_account:
            invoice_id.partner_bank_id = bank_account
    ...
    def _get_concepts(self, xml, journal, supplier, account_id, taxes):
        invoice_line_ids = []
        ...
        for idx, rec in enumerate(xml.Conceptos.Concepto):
            ...
            invoice_line_ids.append((0, 0, {
                ...
                'uom_id': uom_id.id,
                'invoice_line_tax_ids': [(6, 0, line_tax_ids)],
                'price_unit': float(price),
                'discount': discount,
            }))
            # Exception for TELMEX
            if xml.Emisor.get('Rfc', False) == 'TME840315KT6':
                self._update_lines(invoice_line_ids, line_taxes)
        ecc12 = self._get_fuel_complement(xml)
        if ecc12 is not None:
            invoice_line_ids.extend(self._get_concepts_ecc12(ecc12, supplier))
        return invoice_line_ids

    def _get_concepts_ecc12(self, cfdi, supplier):
        invoice_line_ids = []
        ...
        try:
            ...
            return {
                'key': False, 'where': 'CreateInvoice',
                'error': [
                    _('Product assigned for ECC not assigned correctly.<br/>')]}
        ...
        default_analytic = self.get_default_analytic(product, supplier)
        taxes = self.with_context(force_taxes_complement=True).get_impuestos(cfdi).get('taxes_ids', {})
        for idx, rec in enumerate(cfdi.Conceptos.ConceptoEstadoDeCuentaCombustible):
            tax = taxes.get(idx)[0] if taxes.get(idx, []) else {}
            quantity = 1.0
            ...
            invoice_line_ids.append((0, 0, {
                ...
                'account_id': account.id,
                'account_analytic_id': default_analytic.analytic_id.id if default_analytic else False,
                'name': '%s - %s' % (rec.get('ClaveEstacion'), rec.get('FolioOperacion')),
                'quantity': float(quantity),
                'invoice_line_tax_ids': [(6, 0, [tax['id'] for tax in taxes.get(idx, [])])],
                'uom_id': product.uom_id.id,
                'price_unit': float(price),
            }))
        return invoice_line_ids
    ...
    def _update_lines(self, lines, line_taxes):
        """Get correct lines for TELMEX
        The CFDI structure is the next:
        - 1 concept with 2 taxes (IEPS - IVA)
        And the amounts are the next (with a example):
            IEPS base 100
            IEPS amount = 100 * 3% = 3
            TAX base = 100 + 3
            TAX amount = 103 * 16%
        Note: Only is supported this case."""
        if len(lines) != 1 or len(line_taxes) != 2:
            return lines
        ieps = sum([tax.get('base') for tax in line_taxes if tax.get('tax') == 'IEPS'])
        ieps_percent = [tax.get('rate') for tax in line_taxes if tax.get('tax') == 'IEPS']
        tax_id = [tax.get('id') for tax in line_taxes if tax.get('tax') == 'IVA']
        iva = sum([tax.get('base') for tax in line_taxes if tax.get('tax') == 'IVA'])
        lines[0][2]['price_unit'] = ieps
        lines.append((0, 0, lines[0][2].copy()))
        lines[-1][2]['price_unit'] = round(iva - ((ieps * ieps_percent[0] / 100) + ieps), 2)
        lines[-1][2]['invoice_line_tax_ids'] = [(6, 0, tax_id)]
        return lines
