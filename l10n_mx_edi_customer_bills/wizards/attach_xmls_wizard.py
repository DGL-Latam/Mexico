...
class AttachXmlsWizard(models.TransientModel):
    ...
    def validate_documents(self, key, xml, account_id):
        ...
        use_default_partner = self.env['ir.config_parameter'].sudo().get_param(
            'force_partner_wizard_customer_bills', '')
        if use_default_partner and self._context.get('partner_id'):
            ...
            domain.extend(['|', ('company_id', '=', False), ('company_id', '=', self.env.user.company_id.id)])
            exist_supplier = (partner_obj.search(domain, limit=1).commercial_partner_id or
    ...
    def create_invoice(
        ...
        if bank_account:
            invoice_data['partner_bank_id'] = bank_account
